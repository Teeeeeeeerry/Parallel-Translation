...
| **站点页面规则** | 站点规则 | `SiteRules`、`src/storage/specialization.ts`(`getSiteRules()` 生效站点规则;`UserSiteRules` 用户规则,存 storage.local,页面先 `siteRulesReady()` 载入)、`src/storage/builtin-site-rules.ts`(内置规则数据);排除在全页翻译的 `collect()` 与逐段翻译的 `closestUnit()` 两个入口生效;代码层为 `src/dom/compat.ts` |
