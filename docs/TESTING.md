...
### 注入 UI 的指针交互不归单元层，必须优先上 E2E

这一条是阶段 5 验收付出代价换来的，写在这里以免重蹈。段落悬停按钮的 DoD「能顺利移动到按钮并点击」被 jsdom 用例连判两轮通过，实机上却完全不可用，三轮才修干净（[phase-5/DoDR-3](DoD-report/phase-5/DoDR-3.md)）。jsdom 缺的不是某个 API，而是**判据本身依赖的东西**：

| 缺什么 | 漏掉了什么缺陷 |
|---|---|
| 真实指针轨迹与移动耗时 | 按钮追鼠标逐段跳变；从段落移到按钮的时间预算不够 |
| 布局（`getBoundingClientRect` 恒为 0，只能装替身） | 按钮被定位到视口外；压住正文 |
| 真实的 `visibilityState` 与 rAF 节流 | 后台标签页里 rAF 不回调，按钮卡在全透明 |

因此：**凡是判据里出现“看得见”“点得到”“不遮挡”“不闪”的，一律归 E2E 层**，不要用 jsdom 的事件派发去近似 —— 它只能证明处理器接线正确，证明不了交互可用。单元层对这类组件只负责纯逻辑（状态机迁移、选择器判定、清理函数是否摘干净监听器）。

对应地，E2E fixture 除了已列的结构特征，还需覆盖版式特征：受限列宽与全宽两种正文、贴近视口边缘的段落、长于视口的段落。

### 样式与 manifest 的判据要查产物，不能查源码

这一条是阶段 7 验收换来的，同样写在这里以免重蹈。两个缺陷都是“源码看着没问题、产物里根本不存在”：

| 缺陷 | 源码里看到的 | 产物里实际的 |
|---|---|---|
| 样式预设实时预览失效 | `presets.css` 六个预设写得好好的 | `options.css` 从没 `@import` 它，产物 CSS 里一条预设规则都没有 |
| 扩展在 Chrome 里装不上 | `_locales/` 三份 `messages.json` 齐备 | manifest 没有 `default_locale`，Chrome 直接拒绝加载 |

共同点是**缺陷位于源码之间的接线处**，而不是任何一个文件内部。查单个文件永远查不出来。因此：

- CSS 判据断言 `.output/**/assets/*.css` 里有没有那条规则，不断言 `src/styles/*.css` 里写没写
- manifest 判据断言 `.output/*/manifest.json`，不断言 `wxt.config.ts`
- 模块可达性判据断言产物 chunk 的引用关系（构建器会拆共享 chunk，“代码在不在自己这个文件里”不是有效判据）

这类断言便宜到可以每次 `pnpm build` 后无条件跑一遍，没有理由省。
...
fixture 不是“简化版测试”，而是**把各站点的结构特征提炼成可控样本**：shadow DOM 嵌套、同源 iframe、无限滚动、SPA 路由、激进 CSS reset，以及注入 UI 定位所依赖的版式特征（受限列宽 / 全宽正文、贴边段落、超长段落）。这些特征稳定不变，站点的皮肤才是易变的。
